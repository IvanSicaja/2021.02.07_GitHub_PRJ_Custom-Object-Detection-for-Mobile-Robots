import time
import pstats           # Used for cProfiler converting in human-readable file
import cProfile         # Used for every function code speed analise
from pstats import SortKey
import snakeviz         # Used for cProfile stats visualization
import cv2
import numpy as np
...
def initialiseTrackbars():
    ...
    cv2.createTrackbar('MinCntSize', 'Sliders', 0, 500000, nothing)  # Create trackbar
    cv2.createTrackbar('MaxCntSize', 'Sliders', 0, 500000, nothing)  # Create trackbar
    cv2.setTrackbarPos('MinCntSize', 'Sliders', 7000)  # Set default trackbars values
    cv2.setTrackbarPos('MaxCntSize', 'Sliders', 500000)  # Set default trackbars values

def extractAndFitObjectFromContourToCNNModel(onImgDrawContours, imgThreshold, cnt ):
    ...
    print("\nNumber of edges for selected contour: ", len(detectedNumberOfEdgesOnContur))  # Print the number of edges on the contour
    ...
    print("Extracted contour > x,y,width,height : " + str(x) + "-" + str(y) + " " + str(w) + "-" + str(h))
    ...
    cv2.imshow("Image of extracted object", imgObjectSelected)  # Show extracted object from extended bounding box in cv2 window
    ...
    blackBcgImg = cv2.imread("../../assets/media/figures/developmental_phase_images/Black_background.png")[:, :, 0]  # Load image with one channel "grayscale" #TODO: Add this before while loop
    ...
    print("imgToImgCoordinates > Y,Y+height,X,X+width: ", str(drawImgToImagStartCordinateY) + "-" + str(drawImgToImagStartCordinateY + heght) + " " + str(drawImgToImagStartCordinateX) + "-" + str(drawImgToImagStartCordinateX + width))
...
def predictCharacterFromPreprocessedExtracedImage(model,imgGoToCnn):
    # ----------> Send extracted object to the trained CNN model and predict object value <----------#
    pred = model.predict(imgGoToCnn)
    print("Predicted class is: ", pred.argmax(), end=' . ')  # NOTE: The most important part of the code -> pred.argmax() predict class for input image
...
def characterRecognition():
    ...
    videoInput=True
    #TODO: Add contour area min and max trackbars
    # ---------------------------------------#

    initialiseTrackbars() # Initialize GUI
    ...
    model = load_model('../../models/trained/final/A-Z_0-9_Model_Batchsize=500__Epoch=60__Accuracy=77.5.h5')  # Load trained wanted model
    model.summary()  # Load the model summary, can be useful to see CNN layers structure

    # ----------> Camera setting if you want to use video as input <----------#
    if videoInput==True:
        frameWidth = 1080
        frameHeight = 720
    ...
    while whileLoopCondition:
        try:
            ...
            MinCntSize=cv2.getTrackbarPos('MinCntSize', 'Sliders')  # Read value from trackbar
            MaxCntSize=cv2.getTrackbarPos('MaxCntSize', 'Sliders')  # Read value from trackbar

            # ----------> Define image/video source from witch we went recognise object <----------#
            # NOTE: ".." in the path means go to the parent's directory
            startTimeSpentPerImage=time.time()
            startTimeFrameRead=time.time()
            if videoInput==True:
                success, img = cap.read()

            if videoInput==False:
                path = '../../assets/media/figures/developmental_phase_images/H_S_U.png'  # Wanted image
                img = cv2.imread(path)  # Read the wanted image
            endTimeFrameRead = time.time()
            TIME_NEEDED_FOR_FRAME_READ = endTimeFrameRead-startTimeFrameRead
            ...
            startTimeContourExtraction = time.time()
            ...
            endTimeContourExtraction = time.time()

            for cnt in contours:  # Individually access to every contour
                area = cv2.contourArea(cnt)  # Calculate area of every image
                print("")
                print("Contours area: ", str(area) + ' px',end = '')  # Print the size in pixels of contour

                if area > MinCntSize and area<MaxCntSize:

                    startTimeExtractedObjectPreprocessing=time.time()
                    imgGoToCnn=extractAndFitObjectFromContourToCNNModel(onImgDrawContours, imgThreshold, cnt)
                    endTimeExtractedObjectPreprocessing = time.time()

                    startTimeCharacterRecognition = time.time()
                    predictCharacterFromPreprocessedExtracedImage(model, imgGoToCnn)
                    endTimeCharacterRecognition = time.time()

                    # ----------> Add the text description to every image <----------#
                    startTimeImageStack= time.time()
                    ...
                    endTimeImageStack = time.time()

                    TIME_NEEDED_FOR_CONTUR_EXTRACTION = endTimeContourExtraction - startTimeContourExtraction
                    TIME_NEEDED_FOR_EXTRACTED_OBJECT_PREPROCESSING = endTimeExtractedObjectPreprocessing - startTimeExtractedObjectPreprocessing
                    TIME_NEEDED_FOR_CHARACTER_RECOGNITION = endTimeCharacterRecognition - startTimeCharacterRecognition
                    TIME_NEEDED_FOR_IMAGE_STACK = endTimeImageStack- startTimeImageStack
                    print("TIME_NEEDED_FOR_FRAME_READ:                     ", TIME_NEEDED_FOR_FRAME_READ)
                    print('TIME_NEEDED_FOR_CONTUR_EXTRACTION:              ', TIME_NEEDED_FOR_CONTUR_EXTRACTION)
                    print('TIME_NEEDED_FOR_EXTRACTED_OBJECT_PREPROCESSING: ', TIME_NEEDED_FOR_EXTRACTED_OBJECT_PREPROCESSING)
                    print('TIME_NEEDED_FOR_CHARACTER_RECOGNITION:          ', TIME_NEEDED_FOR_CHARACTER_RECOGNITION)
                    print('TIME_NEEDED_FOR_IMAGE_STACK:                    ', TIME_NEEDED_FOR_IMAGE_STACK)
                    print("Number of iteration:", iterationCounter)  # Print number of calls of getContours() function

                    print("---------------------------------------------------------------------")
                    print("PRESS 'SPACE' FOR NEXT STEP <=> PRESS 'ESC' for end script.")
                    print("NOTE: STEP BY STEP mode need to be turned on.")

                    # ----------> Conditions for stapBystep and continuous mode <----------#
                    if stepByStepMode==True:
                        # NOTE: If you want to show every detected image step by step uncomment the block bellow, for the next image you need to press any key, for the exit from the program press "ESC" key two times
                    ...
                    if stepByStepMode==False:
                        # NOTE: If you want continuous mode uncomment the block bellow
                    ...
                    # print(". Contour size isn't in defined pixel size range.")
                    lj='a'


            endTimeSpentPerImage = time.time()
            TIME_SPENT_PER_IMAGE = endTimeSpentPerImage - startTimeSpentPerImage
            print('\nTIME_SPENT_PER_IMAGE----------------------------->', TIME_SPENT_PER_IMAGE)

        except:
            print("-----------------> EXCEPT BLOCK IS TRIGERED <-------------------")


# ---------> Here call main program function <--------#
# characterRecognition()
# print("\nSCRIPT IS CLOSED")

if __name__=="__main__":

    cProfile.run('characterRecognition()',"cProfilerFiles/output.prof")

    with open("cProfilerFiles/output_time.txt","w") as f:
        p = pstats.Stats("cProfilerFiles/output.prof", stream=f)
        p.sort_stats("time").print_stats()
    with open("cProfilerFiles/output_calls.txt","w") as f:
        p = pstats.Stats("cProfilerFiles/output.prof", stream=f)
