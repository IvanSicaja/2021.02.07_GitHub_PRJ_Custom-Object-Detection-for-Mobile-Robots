...
while True:                                                                                              # Loop our program
    ...
    # ----------> Send images to the object extraction and recognition function <----------#
    ...
    def getContours(img, onImgDrawContours, fromImgExtractObject, iterationCounter):
        ...
        for cnt in contours:  # Individually access to every contour
            ...
            if area > 17:
                ...
                cv2.rectangle(onImgDrawContours, (detectedObjectStartCoordinateX, detectedObjectStartCoordinateY),
                              (detectedObjectStartCoordinateX + w, detectedObjectStartCoordinateY + h), (0, 255, 0),
                              2)  # Draw the rectangle around the contour i.e. around detected object= bounding box
                addTextWithBackgroundRectangleToImage(onImgDrawContours, "Selection", detectedObjectStartCoordinateX,
                                                      detectedObjectStartCoordinateY, 2, (0, 255, 00),
                                                      0.7)  # Add text with background in upper left corner on bounding box
                ...
                if w > h:  # Looking for the larger object dimension
                    extendInitialObjectSelection = int(
                        boundingBoxExtensionPercent * w)  # Calculate the number of pixel needed for the extension
                else:
                    extendInitialObjectSelection = int(
                        boundingBoxExtensionPercent * h)  # Calculate the number of pixel needed for the extension

                textBelowBondingBoxStartCoordinateX = x - int(
                    extendInitialObjectSelection / 2)  # Starting coordinates for writing the text bellow the bounding box
                textBelowBondingBoxStartCoordinateY = y + h + int(
                    extendInitialObjectSelection / 2)  # Starting coordinates for writing the text bellow the bounding box

                # Start and end coordinates for the extended bounding box
                boundingBoxStart_EndCoordinates = [x - int(extendInitialObjectSelection / 2),
                                                   # Coordinate X upper left corner of the extended bounding box
                                                   y - int(extendInitialObjectSelection / 2),
                                                   # Coordinate Y upper left corner of the extended bounding box
                                                   x + w + int(extendInitialObjectSelection / 2),
                                                   # Coordinate X lower right corner of the extended bounding box
                                                   y + h + int(
                                                       extendInitialObjectSelection / 2)]  # Coordinate Y lower right corner of the extended bounding box

                cv2.rectangle(onImgDrawContours,
                              (x - int(extendInitialObjectSelection / 2), y - int(extendInitialObjectSelection / 2)), (
                              x + w + int(extendInitialObjectSelection / 2),
                              y + h + int(extendInitialObjectSelection / 2)), (0, 255, 0),
                              2)  # Draw the extended bounding box around the contour
                addTextWithBackgroundRectangleToImageBellowBoundingBox(onImgDrawContours, "Extension",
                                                                       textBelowBondingBoxStartCoordinateX,
                                                                       textBelowBondingBoxStartCoordinateY, 2, 2,
                                                                       (0, 255, 00),
                                                                       0.7)  # Add text with background in lower left corner on bounding box
                ...
                imgObjectSelected = fromImgExtractObject[  # Crop space inner extended bounding box
                                    ...
                                    # X inner rage of extended bounding box
                ...
                cv2.imshow("Image of object",
                           imgObjectSelected)  # Show extracted object from extended bounding box in cv2 window
                ...
                imgObjectSelectedResizedClrInverted = np.invert(np.array(
                    [imgObjectSelectedResized]))  # Inverting image pixels value e.g: black color to white or reverse
                print('imgObjectSelectedResizedClrInverted',
                      imgObjectSelectedResizedClrInverted.shape)  # Print image shape
                ...
                blackBcgImg = cv2.imread("../../developmental_phase_images/Black_background.png")[:, :,
                              0]  # Load image with one channel "grayscale"
                ...
                blackBcgImg[drawImgToImagStartCordinateY: drawImgToImagStartCordinateY + heght,
                drawImgToImagStartCordinateX: drawImgToImagStartCordinateX + width] = imgObjectSelectedResizedClrInverted  # Paste image in to another image
                print("imgToImgCoordinates: ", drawImgToImagStartCordinateY, drawImgToImagStartCordinateY + heght,
                      drawImgToImagStartCordinateX, drawImgToImagStartCordinateX + width)
                ...
                print("Predicted class is: ",
                      pred.argmax())  # NOTE: The most important part of the code -> pred.argmax() predict class for input image
    ...
    imgStack = stackImages(round(scalePreviewImagesValue*0.1, 1),
                           ([img, imgGray, imgBlur],            # Add all the images on the one window
